package com.arkalgud.scenemorph.ar.surfacemorpher.utilities;

//Copied this for my own understanding
//Source: https://github.com/claywilkinson/arcore-android-sdk/blob/sceneform-samples/samples/cloud_anchor_java/app/src/main/java/com/google/ar/core/examples/java/cloudanchor/sceneform/PointCloudNode.java
...
//Rendered the feature point cloud created by ARCore as Node
public class PointCloudNode extends Node {

    private long timestamp; //idk
    private Vertex[] ptbuffer; //idk
    private int[] indexbuffer; //idk

    private CompletableFuture<Material> materialHolder; //idk
    ...
    /**
     * Update the renderable for the point cloud.
     * This creates a small quad for each feature point.
     *
     * @param cloud the ARCore point cloud.
     */
    @SuppressWarnings({"AndroidApiChecker", "FutureReturnValueIgnores"})
    public void update(PointCloud cloud){
        ...
        //If the same cloud, or if the material hasn't loaded skip this
        if(this.timestamp != cloud.getTimestamp() && materialHolder.getNow(null) != null){
            ...
            //no featured in the cloud
            if(numFeatures < 1){
                setRenderable(null);
                return;
            }

            //The points are drawn as a 4 sided pyramid
            //4 points per feature
            int vertextPerFeature = 4;
            int numFaces = 4;
            int numPoints = numFeatures * vertextPerFeature;
            int vertextPerFace = 3;

            //draw a triangle per face (4 triangles) per feature.
            int indexPerFeature = numFaces * vertextPerFace;

            int numIndices = numFeatures * indexPerFeature;

            //allocate a buffer on the high water mark (idk what this means rn)
            if(ptbuffer ==  null || ptbuffer.length < numPoints){
                ptbuffer = new Vertex[numPoints];
                indexbuffer = new int[numIndices];
            }

            //assigning points of the pyramid
            ...
            for(int i = 0; i < buf.limit() / 4; i++){
                ...
                int vertexBase = i * vertextPerFeature;
                // Create vertices
                //TODO: what is the use of setNormal
                ...
                // The indices of the triangle need to be listed counterclockwise
                // As appears when facing the front side of the face
